...
interface CartContextValue {
  ...
  removeItem: (inventoryId: number) => Promise<void>;
  updateQuantity: (inventoryId: number, qty: number) => Promise<void>;
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const addItem = useCallback(async (newItem: CartItem) => {
    ...
    try {
      ...
      const existing = items.find((i) => i.inventoryId === newItem.inventoryId);
      const newQty = existing ? existing.quantity + newItem.quantity : newItem.quantity;
      await apiClient.post('/orders/cart/items/', {
        inventory_id: newItem.inventoryId,
        quantity: newQty
      });
  ...
  const removeItem = useCallback(async (inventoryId: number) => {
    if (!isAuthenticated || user?.role !== 'STUDENT') return;
    try {
      await apiClient.delete('/orders/cart/items/', { data: { inventory_id: inventoryId } });
      await fetchCart();
    } catch (err) {
  ...
  const updateQuantity = useCallback(async (inventoryId: number, qty: number) => {
    if (!isAuthenticated || user?.role !== 'STUDENT') return;
    try {
      await apiClient.post('/orders/cart/items/', {
  ...
  const clearCart = useCallback(async () => {
    ...
    try {
      ...
      for (const item of items) {
        await apiClient.delete('/orders/cart/items/', { data: { inventory_id: item.inventoryId } });
      }
      setItems([]);
    } catch (err) {
