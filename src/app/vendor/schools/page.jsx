  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
        {school.is_approved && (
          <Link
            href={`/vendor/products?school=${school.id}`}
...
export default function VendorSchoolsPage() {
  ...
  const filtered =
    filter === "all"
      ? schools
      : schools.filter((s) => s.approval_status === filter);
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p className="empty-state-title">
            No schools {filter !== "all" ? `with status "${filter}"` : "yet"}
          </p>
          <p className="empty-state-desc">
            Wait for the admin to assign a school to you to start adding
            products
          </p>
        </div>
      ) : (
        ...
        >
          {filtered.map((s) => (
            <SchoolCard key={s.id} school={s} onRefresh={fetchSchools} />
          ))}
        </div>
      )}
    </div>
