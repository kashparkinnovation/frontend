...
export default function VendorOrdersPage() {
  ...
  const updateStatus = async () => {
    ...
    setUpdating(true);
    try {
      await apiClient.patch(`/orders/${selected.id}/status/`, {
        status: newStatus,
      });
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p className="empty-state-desc">
              Orders placed by students will appear here
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <button
                  onClick={async () => {
                    try {
                      const res = await apiClient.get(
                        `/orders/${selected.id}/delivery-slip/`,
                        { responseType: "blob" },
                      );
                      const url = window.URL.createObjectURL(
                        new Blob([res.data]),
                      ...
                      link.setAttribute(
                        "download",
                        `DeliverySlip_${selected.order_number}.pdf`,
                      );
                      document.body.appendChild(link);
                      link.click();
                      link.parentNode?.removeChild(link);
                    } catch (err) {
                      showToast("Failed to download delivery slip", "error");
                    }
                  }}
                  style={{
                    flex: 1,
