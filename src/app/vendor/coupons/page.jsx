  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      if (form.min_order_amount)
        payload.min_order_amount = form.min_order_amount;
  ...
  const handleToggle = async (coupon) => {
    ...
    try {
      const { data } = await apiClient.patch(
        `/orders/coupons/${coupon.id}/toggle/`,
      );
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          >
            {coupons.filter((c) => c.is_active && !c.is_expired).length} active
            coupons
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p className="empty-state-desc">
              Create discount coupons to attract more orders
            </p>
            <button
                    ...
                    <td onClick={(e) => e.stopPropagation()}>
                      <div style={{ display: "flex", gap: "0.35rem" }}>
                        <button
                          ...
                          style={{
                            background: c.is_active ? "#fef3c7" : "#d1fae5",
                            color: c.is_active ? "#92400e" : "#065f46",
                            border: "none",
                          }}
                        >
                          {toggling === c.id
        ...
        <form onSubmit={handleCreate}>
          ...
          <div className="form-group" style={{ margin: "0 0 1rem" }}>
            ...
            <span className="form-hint">
              Will be auto-uppercased. Must be unique.
            </span>
          </div>
          <div className="form-group" style={{ margin: "0 0 1rem" }}>
        ...
        {selected && (
          <>
            ...
            <DrawerRow
              label="Type"
              value={
                selected.discount_type === "percent" ? "Percentage" : "Flat"
              }
            />
            <DrawerRow
              label="Value"
            ...
            />
            {selected.min_order_amount && (
              <DrawerRow
                label="Min Order"
                value={`₹${selected.min_order_amount}`}
              />
            )}
            {selected.max_discount && (
              <DrawerRow label="Max Cap" value={`₹${selected.max_discount}`} />
            )}

            <DrawerSection title="Usage" />
            <DrawerRow
              label="Times Used"
              value={`${selected.used_count}${selected.max_uses ? ` / ${selected.max_uses}` : ""}`}
            />
            <DrawerRow
              label="Valid From"
              value={new Date(selected.valid_from).toLocaleDateString("en-IN")}
            />
            <DrawerRow
              label="Valid Until"
              value={
                selected.valid_until
                  ? new Date(selected.valid_until).toLocaleDateString("en-IN")
                  : "∞ No expiry"
              }
            />
            <DrawerRow
              label="Status"
              value={<StatusBadge status={couponStatus(selected)} />}
            />

            {selected.school_names.length > 0 && (
              <>
                <DrawerSection title="Restricted to Schools" />
                {selected.school_names.map((s, i) => (
                  <DrawerRow key={i} label="" value={`🏫 ${s}`} />
                ))}
              </>
            )}
