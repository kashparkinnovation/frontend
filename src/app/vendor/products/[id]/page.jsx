...
export default function ProductDetailPage() {
  ...
  const fetchProduct = React.useCallback(() => {
    apiClient
      .get(`/store/vendor/products/${id}/`)
      .then((r) => {
        setProduct(r.data);
        const primary =
          r.data.images?.find((i) => i.is_primary) || r.data.images?.[0];
  ...
  const handleSetPrimary = async (imageId) => {
    try {
      await apiClient.patch(
        `/store/vendor/products/${id}/images/${imageId}/set-primary/`,
      );
  ...
  const toggleActive = async () => {
    if (!product) return;
    try {
      const r = await apiClient.patch(`/store/vendor/products/${id}/`, {
              ...
              {product.images.map((img) => (
                <div
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    border:
                      activeImg === resolveUrl(img.image)
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div className="card">
            ...
            >
              ...
              {product.school_name && (
                <span className="chip">🏫 {product.school_name}</span>
              )}
