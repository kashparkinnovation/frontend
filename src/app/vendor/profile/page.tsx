  ...
  useEffect(() => {
    apiClient.get('/vendors/profile/')
      .then((r) => {
        ...
        if (v.logo) setLogoPreview(v.logo.startsWith('http') ? v.logo : `${API_BASE}${v.logo}`);
      })
      .catch(console.error)
    ...
    <div style={{ maxWidth: 640, margin: '0 auto' }}>
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          <span className={`badge ${vendor?.is_approved ? 'badge-success' : 'badge-warning'}`}>
        ...
        <form onSubmit={handleSave}>
          ...
          <div className="form-grid" style={{ marginBottom: '1.25rem' }}>
            <div className="form-group">
              ...
              <input className="input" name="business_name" value={form.business_name} onChange={handleChange} required />
            </div>
            <div className="form-group">
              <label className="form-label">GST Number</label>
              <input className="input" name="gst_number" value={form.gst_number} onChange={handleChange} placeholder="22AAAAA0000A1Z5" />
            </div>
          </div>
          ...
          <div className="form-grid" style={{ marginBottom: '1.5rem' }}>
            <div className="form-group">
              ...
              <input className="input" value={vendor?.user_email ?? ''} readOnly style={{ background: 'var(--color-bg)', cursor: 'not-allowed' }} />
            </div>
            <div className="form-group">
              <label className="form-label">Full Name</label>
              <input className="input" value={vendor?.user_name ?? ''} readOnly style={{ background: 'var(--color-bg)', cursor: 'not-allowed' }} />
            </div>
          </div>
