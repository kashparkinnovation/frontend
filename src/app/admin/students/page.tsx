...
export default function AdminVerificationQueuePage() {
  ...
  const handleAction = async (action: 'approve' | 'reject') => {
    ...
    setActioning(true);
    try {
      await apiClient.patch(`/students/admin/verification-requests/${selected.id}/action/`, { action, review_note: reviewNote });
      alert(`Request physically ${action === 'approve' ? 'Approved' : 'Rejected'} by Admin!`);
      setSelected(null);
      fetchQueue();
    } catch {
      alert('Action failed');
  ...
  return (
    <div style={{ padding: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h1 style={{ margin: 0, fontSize: '1.875rem', fontWeight: 800 }}>Student Requests</h1>
      ...
      <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        {loading ? (
          <div style={{ padding: '2rem', textAlign: 'center', color: '#64748b' }}>Loading requests...</div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                  ...
                  <th style={{ padding: '1rem', color: '#64748b', fontSize: '0.875rem' }}>Student & School</th>
                  <th style={{ padding: '1rem', color: '#64748b', fontSize: '0.875rem' }}>Class / Roll</th>
                  <th style={{ padding: '1rem', color: '#64748b', fontSize: '0.875rem', textAlign: 'center' }}>ID Card</th>
                ...
                {requests.length === 0 ? (
                  ...
                  >
                    ...
                    <td style={{ padding: '1rem' }}>
                      <div style={{ fontWeight: 600, color: '#0f172a' }}>{req.student.student_name}</div>
                      <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '0.2rem' }}>{req.student.school_name}</div>
                    </td>
                    <td style={{ padding: '1rem', color: '#475569' }}>
                      {req.student.class_name} {req.student.section} - {req.student.roll_number}
                    </td>
                    <td style={{ padding: '1rem', textAlign: 'center' }}>
                      {req.id_card ? <span title="ID card attached">📎</span> : <span style={{ color: '#cbd5e1' }}>—</span>}
                    </td>
                    <td style={{ padding: '1rem' }}>
      ...
      >
        {selected && (
          <>
            ...
            <DrawerRow label="Submitted Date" value={new Date(selected.created_at).toLocaleString()} />
            <DrawerRow label="Global Status" value={<StatusBadge status={selected.status} />} />
              ...
              <div>
                <a href={selected.id_card} target="_blank" rel="noopener noreferrer">
                  <img 
                    ...
                    alt="Student ID card" 
                    style={{ maxWidth: '100%', maxHeight: '200px', objectFit: 'contain', borderRadius: '8px', border: '1px solid #e2e8f0', display: 'block', cursor: 'zoom-in' }} 
                  />
                </a>
                <p style={{ margin: '0.4rem 0 0', fontSize: '0.75rem', color: '#94a3b8' }}>Click to view full image in new tab</p>
              </div>
            ) : (
              <p style={{ margin: 0, fontSize: '0.875rem', color: '#94a3b8' }}>No official ID card attached</p>
            )}
            ...
            {selected.status === 'pending' && (
              <>
                ...
                <textarea
                  ...
                  rows={3}
                  style={{ width: '100%', padding: '0.75rem', border: '1px solid #cbd5e1', borderRadius: '6px', fontSize: '0.875rem', resize: 'vertical', fontFamily: 'inherit' }}
                />
                <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1rem' }}>
                  <button 
                    onClick={() => handleAction('approve')} 
                    disabled={actioning} 
                    style={{ flex: 1, padding: '0.75rem', background: '#22c55e', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: actioning ? 'wait' : 'pointer' }}
                  >
                  ...
                  <button 
                    ...
                    disabled={actioning} 
                    style={{ flex: 1, padding: '0.75rem', background: '#fee2e2', color: '#b91c1c', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: actioning ? 'wait' : 'pointer' }}
                  >
