  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.75rem', fontWeight: 800, margin: '0 0 0.375rem', letterSpacing: '-0.02em' }}>
          Welcome back, {user?.first_name ?? user?.email?.split('@')[0]} 👋
        </h1>
        <p style={{ color: '#64748b', margin: 0 }}>Manage your student profiles and order uniforms below.</p>
      </div>
        ...
        {[
          ...
          <Link key={stat.label} href={stat.href} style={{ textDecoration: 'none' }}>
            <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '14px', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.375rem', transition: 'box-shadow 0.15s' }}
              onMouseEnter={(e) => ((e.currentTarget as HTMLDivElement).style.boxShadow = '0 4px 14px rgb(0 0 0/0.07)')}
              ...
              <span style={{ fontSize: '0.8125rem', color: '#64748b', fontWeight: 500 }}>{stat.label}</span>
            </div>
          </Link>
        ))}
        <Link href="/browse" style={{ textDecoration: 'none' }}>
          <div style={{ background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', borderRadius: '14px', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.375rem', color: 'white', cursor: 'pointer', transition: 'opacity 0.15s' }}
            ...
            <span style={{ fontSize: '0.8125rem', opacity: 0.85 }}>Explore schools</span>
          </div>
        </Link>
      </div>
      ...
      {students.length === 0 && (
        <div style={{ position: 'relative', borderRadius: '16px', overflow: 'hidden', marginBottom: '2rem', display: 'flex', border: '1px solid #e2e8f0', background: 'white', boxShadow: '0 4px 14px rgb(0 0 0/0.05)' }}>
          ...
          <div style={{ width: '40%', minWidth: 250, position: 'relative' }}>
            <img src="https://images.unsplash.com/photo-1503676260728-1c00da094a0b?auto=format&fit=crop&w=800&q=80" alt="Students" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, white 0%, transparent 100%)' }} />
          </div>
        </div>
      )}
      ...
      {students.length > 0 && (
        <div style={{ marginBottom: '2rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <h2 style={{ margin: 0, fontSize: '1.125rem', fontWeight: 700 }}>Your Students</h2>
            <Link href="/store/students/new" style={{ fontSize: '0.875rem', color: '#4f46e5', fontWeight: 600, textDecoration: 'none' }}>+ Add Student</Link>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem' }}>
            {students.map((s) => (
              <div key={s.id} style={{ background: 'white', border: `1.5px solid ${activeStudent?.id === s.id ? '#4f46e5' : '#e2e8f0'}`, borderRadius: '14px', padding: '1.25rem', transition: 'all 0.15s' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', marginBottom: '1rem' }}>
                  <div style={{ width: 44, height: 44, borderRadius: '50%', background: s.is_verified ? '#d1fae5' : '#fef3c7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  <Link href={`/store/students/${s.id}`}
                    style={{ padding: '0.5rem 0.75rem', background: '#f8fafc', color: '#64748b', borderRadius: '8px', textDecoration: 'none', fontSize: '0.8125rem', border: '1px solid #e2e8f0' }}>
                    ⚙️
                  </Link>
                </div>
              </div>
      ...
      <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '14px', padding: '1.5rem' }}>
        <h2 style={{ margin: '0 0 1rem', fontSize: '1.0625rem', fontWeight: 700 }}>Quick Actions</h2>
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
          {[
            { label: '+ Add Student Profile', href: '/store/students/new', bg: '#eef2ff', color: '#4f46e5' },
            { label: '📦 View All Orders', href: '/store/orders', bg: '#f0fdf4', color: '#166534' },
            { label: '🏫 Browse All Schools', href: '/browse', bg: '#f8fafc', color: '#64748b' },
          ].map(({ label, href, bg, color }) => (
            <Link key={href} href={href} style={{ display: 'inline-block', background: bg, color, fontWeight: 600, padding: '0.625rem 1.1rem', borderRadius: '8px', textDecoration: 'none', fontSize: '0.875rem' }}>
              {label}
            </Link>
          ))}
        </div>
