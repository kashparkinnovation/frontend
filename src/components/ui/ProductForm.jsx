...
export default function ProductForm({
  ...
  useEffect(() => {
    if (!productId) return;
    apiClient
      .get(`/store/vendor/products/${productId}/`)
      .then((r) => {
        const p = r.data;
        setForm({
  ...
  const saveDetails = async () => {
    ...
    try {
      const payload = {
        ...
        school_commission_percent: form.school_commission_percent
          ? parseFloat(form.school_commission_percent)
          : null,
      };
      let id;
      if (isEdit && savedProductId) {
        const r = await apiClient.patch(
          `/store/vendor/products/${savedProductId}/`,
          payload,
        );
        id = r.data.id;
  ...
  const handleImageUpload = useCallback(
    async (file) => {
      if (!savedProductId) {
        toast("Save product details first", "error");
        return;
      }
      setUploading(true);
      try {
        ...
        const r = await apiClient.post(
          `/store/vendor/products/${savedProductId}/images/`,
          fd,
          {
    ...
    async (imageId) => {
      ...
      try {
        await apiClient.delete(
          `/store/vendor/products/${savedProductId}/images/${imageId}/`,
        );
  ...
  const handleSetPrimary = useCallback(
    async (imageId) => {
      ...
      try {
        const r = await apiClient.patch(
          `/store/vendor/products/${savedProductId}/images/${imageId}/set-primary/`,
        );
        setImages((prev) =>
  ...
  const saveVariants = async () => {
    ...
    try {
      ...
      if (!toSave.length) {
        toast("Add at least one variant", "error");
        setSaving(false);
        return;
      }
      const payload = toSave.map((v) => ({
        ...
        school_commission_percent: v.school_commission_percent
          ? parseFloat(v.school_commission_percent)
          : null,
      }));
      const r = await apiClient.post(
        `/store/vendor/products/${savedProductId}/inventory/`,
        { variants: payload },
      );
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      <div className="page-header">
        ...
        {savedProductId && (
          <Link
            href={`/vendor/products/${savedProductId}`}
        ...
        <div className="card">
          ...
          <div className="form-grid" style={{ marginBottom: "1.25rem" }}>
            <div className="form-group" style={{ gridColumn: "1 / -1" }}>
              ...
              <input
                ...
                required
                placeholder="e.g. School White Button-Down Shirt"
              />
            </div>
          ...
          <div className="form-grid" style={{ marginBottom: "1.25rem" }}>
            ...
            <div className="form-group">
              ...
              <input
                ...
                value={form.school_commission_percent}
                onChange={handleChange}
                min="0"
                step="0.01"
                placeholder="0.00"
          ...
          <div className="form-group" style={{ marginBottom: "1.25rem" }}>
            ...
            <textarea
              ...
              onChange={handleChange}
              placeholder="Describe the product — fabric, design, purpose…"
              rows={4}
            />
          </div>
          ...
          <div className="form-grid" style={{ marginBottom: "1.25rem" }}>
            ...
            <div className="form-group">
              ...
              <input
                ...
                value={form.tags}
                onChange={handleChange}
                placeholder="summer, formal, cotton (comma-separated)"
              />
            </div>
          </div>
        ...
        <div className="card">
          ...
          {!savedProductId ? (
            ...
            <>
              ...
              >
                Upload multiple photos from different angles. The first uploaded
                image (or the one you mark as primary) will be shown in
                listings.
              </p>
              <ImageUploader
                images={images}
        ...
        <div className="card">
          ...
          {!savedProductId ? (
            ...
            <>
              ...
              >
                Define all size/color combinations with their stock quantity.
                Leave Price Override blank to use the base price.
              </p>
              <VariantBuilder
                variants={variants}
